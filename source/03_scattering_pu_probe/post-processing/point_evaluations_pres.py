...
# ------------------------------------------------------------------- #
# Computes pressure field at a specific coordinate
...
inputfile = 'results/pres_fields.h5'
...
# Output file name
outfile = "point_evaluations_pres.json"
...
h5id = d.HDF5File(mesh.mpi_comm(), inputfile, "r")
...
for frq_count, frq in enumerate(freqs_list):
    ...
    field_re = d.Function(DG0)
    field_im = d.Function(DG0)
    h5id.read(field_re, f"/{finput_field}_re/vector_{frq_count:d}")
    h5id.read(field_im, f"/{finput_field}_im/vector_{frq_count:d}")
